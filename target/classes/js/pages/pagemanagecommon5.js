...
if ((districtcode == "" || districtcode == null) && manageusername != null && manageusername != "" && manageusername != username) {
    //username = getCookie("page_farmusername_5");
    //manageusername = getCookie("watchdogusername");
    $("#a_adminadd").css("display", "none");
    //$("#a_goback").click(function () {
    //    window.location.href = "/PageManageCommon/ManageToNext?clickname=" + manageusername + "&clicknamelevel=4";
    //});
} else {
...
$.ajax({
    ...
    success: function (data) {
        if (data == "failed") {
            ...
            var logintime = "";
            if (data != null) {
                if (data.data3[0].privilegelevel == 5) {
                ...
                //$("#h3_adminname").html("乡级管理员——" + data.data1[0].managername);
                $("#h3_adminname").html("乡级管理员(" + data.data2[0].upmanageraera + ")");
                //$("#li_areasee").css("display", "none");
                for (var i = 0; i < data.data2.length; i++) {
                    logintime = ChangeTimeFormat(data.data2[i].logintime).split(" ")[0];
                    //html += "<tr><td><a onclick=\"nameOnClick(this.id)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "\">" + data.data2[i].managername + "</a></td><td>" + logintime + "</td><td>" + data.data2[i].hamlet + "</td><td>" + data.data2[i].job + "</td><td>" + data.data2[i].chargehamlet + "</td><td><a onclick=\"totalOnClick(this.id,6)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "*\">" + data.data2[i].dogtotalnum + "</a></td><td>" + data.data2[i].officecall + "</td><td>" + data.data2[i].telphonecall + "</td></tr>";
                    html += "<tr><td><a onclick=\"nameOnClick(this.id)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "\">" + data.data2[i].managername + "</a></td><td>" + logintime + "</td><td>" + data.data2[i].managearea + "</td><td>" + data.data2[i].job + "</td><td>" + data.data2[i].managearea + "</td><td><a onclick=\"totalOnClick(this.id,6)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "*\">" + data.data2[i].neckletedtotal + "</a></td><td>" + data.data2[i].feederedtotal + "</td><td>" + data.data2[i].officecall + "</td><td><a href=\"tel:" + data.data2[i].telphonecall + "\">" + data.data2[i].telphonecall + "</a></td></tr>";
                }
            ...
            $("#div_usersearch").click(function () {
                ...
                if ($.trim(managername) == "") {
                    for (var i = 0; i < data.data2.length; i++) {
                        logintime = ChangeTimeFormat(data.data2[i].logintime).split(" ")[0];
                        searchhtml += "<tr><td><a onclick=\"nameOnClick(this.id)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "\">" + data.data2[i].managername + "</a></td><td>" + logintime + "</td><td>" + data.data2[i].managearea + "</td><td>" + data.data2[i].job + "</td><td>" + data.data2[i].managearea + "</td><td><a onclick=\"totalOnClick(this.id,6)\" style=\"cursor:pointer;\" id=\"" + data.data2[i].username + "*\">" + data.data2[i].neckletedtotal + "</a></td><td>" + data.data2[i].feederedtotal + "</td><td>" + data.data2[i].officecall + "</td><td><a href=\"tel:" + data.data2[i].telphonecall + "\">" + data.data2[i].telphonecall + "</td></tr>";
                    }
                    $("#tbody_pagemanagecommon").html(searchhtml);
                    goPage(1, 8);
                    var tempOption = "";
                    ...
                    searchcontent.privilegelevel = 5;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                searchhtml += "<tr><td><a onclick=\"nameOnClick(this.id)\" style=\"cursor:pointer;\" id=\"" + data[i].username + "\">" + data[i].managername + "</a></td><td>" + logintime + "</td><td>" + data[i].managearea + "</td><td>" + data[i].job + "</td><td>" + data[i].managearea + "</td><td><a onclick=\"totalOnClick(this.id,6)\" style=\"cursor:pointer;\" id=\"" + data[i].username + "*\">" + data.data2[i].neckletedtotal + "</a></td><td>" + data.data2[i].feederedtotal + "</td><td>" + data[i].officecall + "</td><td><a href=\"tel:" + data[i].telphonecall + "\">" + data[i].telphonecall + "</td></tr>";
                            }
                            $("#tbody_pagemanagecommon").html(searchhtml);
                            goPage(1, 8);
                            var tempOption = "";
...
function totalOnClick(id, privilegelevel) {
    switch (privilegelevel) {
        case 6:
            id = id.substring(0, id.length - 1);
            //setCookie("page_farmusername_6", id.substring(0, id.length - 1), "s6000");
