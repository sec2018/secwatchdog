...
public class HamletController {
	...
	@ResponseBody
	public String GoToHamlet(@RequestBody(required=false) JSONObject json,HttpServletRequest request) throws Exception {
...
//		data.put("data5", getupuser_page_farmDogFeederList);
	...
	@ResponseBody
	public String GoToHamletPage(@RequestBody String page,HttpServletRequest request) throws Exception {
		HttpSession session=request.getSession();
	...
	@ResponseBody
	public String getCombineNeckletAndFeederDogByNeckletId(@RequestBody String neckletId,HttpServletRequest request) throws Exception {
		HttpSession session=request.getSession();
		if(session.getAttribute("currentUser")==null || session.getAttribute("hamletcode") == null){;
			return null;
		}
